...
TRAVIS_BUILD_CMD='{ "request": { "message": "Compile master for Statping v${VERSION}", "branch": "master", "config": { "go": ["${GOVERSION}"], "os": ["linux"], "env": { "VERSION": "${VERSION}", "secure": "${TRVIS_SECRET}" }, "sudo": "required", "cache": { "directories": ["~/.npm", "~/.cache", "$GOPATH/src/github.com/statping/statping/tmp", "$GOPATH/src/github.com/statping/statping/vendor"] }, "deploy": [{ "on": { "branch": "master" }, "file": "build/*", "api_key": "$TAG_TOKEN", "provider": "releases", "file_glob": true, "skip_cleanup": true }], "matrix": { "fast_finish": true, "allow_failures": [{ "go": "master" }] }, "script": ["travis_wait 30 docker pull crazymax/xgo:${GOVERSION}", "make release"], "install": true, "branches": { "only": ["master"] }, "language": "go", "services": ["docker"], "after_deploy": ["make publish-homebrew"], "after_success": [], "before_deploy": ["git config --local user.name \"hunterlong\"", "git config --local user.email \"info@socialeck.com\"", "git tag v$(VERSION) --force"], "before_script": ["gem install sass"], "notifications": { "email": false }, "go_import_path": "github.com/statping/statping" } } }'

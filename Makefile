...
build-mac:
	...
	do \
		...
		GO111MODULE="on" GOOS=darwin GOARCH=$$arch go build -a -ldflags "-X main.VERSION=${VERSION} -linkmode external -extldflags -static" -buildmode=pie -o releases/statping-darwin-$$arch/statping ${PWD}/cmd || true; \
...
build-win:
	...
	do \
		...
		GO111MODULE="on" GOOS=windows GOARCH=$$arch go build -a -tags "osusergo netgo" -ldflags "-X main.VERSION=${VERSION} -linkmode external -extldflags -static" -buildmode=pie -o releases/statping-windows-$$arch/statping.exe ${PWD}/cmd || true; \
